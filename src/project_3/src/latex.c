...
int gen_tests_schedulability(gui_config *config, FILE *fptr_out) {
    ...
    ttest_params executions[N_ALGORITHMS];

    for (int i = 0; i < N_ALGORITHMS; i++) {
        executions[i].data = NULL;
        executions[i].enabled = false;
    }

    if (config->rm_enabled) {
        sche_data_t * val = testability_rm(config);
        executions[0].data = val;
        executions[0].enabled = true;
    }
    ...
    if (config->rm_enabled || config->edf_enabled || config->llf_enabled){
        write_ttest_slides(fptr_out, executions, false, config->num_tasks);
    }
...
int generate_pdf(gui_config *config, FILE *fptr_out) {
    ...
    fputs("\n\\section{Tests de schedulability}\n", fptr_out);
    gen_tests_schedulability(config, fptr_out);
